...
I prepare a series of functions that identify where duplicated rows differ. 

``` {r}
findDupRows <- function(id, df) {
  return(df[df$horse_id==id,])
}
...
compareDupRows <- function(id, df) {
  outmatrix <- makeOutputMatrix(df)
  duprows <- findDupRows(id, df)
...
compareAllDupRows <- function(dupids, df) {
  outmatrix <- makeOutputMatrix(df)
  for (id in dupids) {
    temp <- compareDupRows(id, df)
    outmatrix <- outmatrix + temp
  }
  return(outmatrix)
}

compareAllDupRows(dup_ids, ped)
```
