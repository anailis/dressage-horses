...
dam_records <- ped %>% 
  filter(horse_id %in% dam_ids$dam | horse_ueln %in% dam_ids$dam) %>%
...
sire_records <- ped %>% 
  filter(horse_id %in% sire_ids$sire | horse_ueln %in% sire_ids$sire) %>%
  select(horse_id, horse_ueln, sex, sex_raw)
...
I will have a brief look into how these males mislabelled as dams (or vice versa) may have arisen. 
