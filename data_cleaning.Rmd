...
``` {r data extraction, warning = F, message = F}
library(tidyverse)
library(RSQLite)
library(lubridate)
...
This leaves us with 280 females, 701 males, and 24 individuals missing sex. 

### Create yob Column

I obtain the year of birth for each individual from the `dob` column. 

``` {r create yob, warning = F}
tail(ped$dob)
...
countRowDiffs <- function(dup_id, df, id_type) {
    # change NAs to character so I can observe differences between rows
    df[is.na(df)] <- "None"
...
#do.call(rbind.data.frame, (map(getDupIds(ped, "horse_id"), countRowDiffs, ped, "horse_id")))
#do.call(rbind.data.frame, (map(getDupIds(ped, "horse_ueln"), countRowDiffs, ped, "horse_ueln")))
getDupIds(ped, "horse_ueln")
getDupIds(ped, "horse_id")
countRowDiffs("208333DW0700794", ped, "horse_ueln")
```
...
indices <- which(ped[["horse_id"]] == '102YA19')
counts <- map_int(indices, function(x) sum(is.na(ped[x,])))
counts
...
Now I return to the dams. There are 41 individuals that are listed twice, each record with a different dam. I wonder if this is because of the mislabelling of sires of dams as dams. 14 dams are also listed in the sire column. Unfortunately, none of these individuals are the sources of ID duplication, so this just identifies even more inconsistency in the dam data. Ultimately, I have chosen to omit the dams from the analysis because of the numerous inconsistencies I found in their data.

``` {r investigate dams}
ped %>%
  filter(dam_ueln %in% sire_ueln & !is.na(dam_ueln) | dam_id %in% sire_id & !is.na(dam_id)) %>%
  filter(dam_ueln %in% dup_uelns)
```
...
This leaves 218 unaccounted for NA scores. They are enriched for "final classifications" which would be an overall position, rather than a single show's score. I remove these, and leave the rest for investigations of missingness during 
...
The sex and yob columns need to be cleaned as for `ped`.
...
The pedigree and the shows column agree about the year of birth and the sex of every horse.
