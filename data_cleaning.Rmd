...
identifyRowsDifferingbyNA <- function(id, dupRows) {
  if (is.na(dupRows[1,"dam_ueln"] == dupRows[2,"dam_ueln"]) & (is.na(dupRows[1,"horse_ueln"] == dupRows[2,"horse_ueln"]) | dupRows[1,"horse_ueln"] == dupRows[1,"horse_ueln"])) {
    return(id)
  }
}

getRowsDifferingByNA <- function(id_list, df, id_type) {
  rowsDifferentByNA <- c()
  for (id in id_list) {
    dupRows <- getDupRows(id, df, id_type)
    rowsDifferentByNA <- append(rowsDifferentByNA, identifyRowsDifferingbyNA(id, dupRows), length(rowsDifferentByNA))
  }
  return(rowsDifferentByNA)
}

uelns_differbyna <- getRowsDifferingByNA(dup_uelns, ped, "horse_ueln")
ped %>%
  filter(horse_ueln %in% uelns_differbyna) %>%
  arrange(horse_ueln)

ids_differbyna <- getRowsDifferingByNA(dup_ids, ped, "horse_id")
ped %>%
...
test <- getDupRows("103HO85", ped, "horse_id")
sum(is.na(test[1,]))
sum(is.na(test[2,]))
...
removeRowsWithLessData <- function(id_list, df, id_type) {
  ...
  for (id in id_list) {
    ...
    if (sum(is.na(df[li[1],])) > sum(is.na(df[li[2],]))) {
      li_toremove <- append(li_toremove, li[1], length(li_toremove))
    }
    if (sum(is.na(df[li[1],])) < sum(is.na(df[li[2],]))) {
      li_toremove <- append(li_toremove, li[2], length(li_toremove))
    }
  }
...
ped <- removeRowsWithLessData(uelns_differbyna, ped, "horse_ueln")
