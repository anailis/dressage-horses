...
identifyRowsDifferingbyNA <- function(id, dupRows) {
  if (sum(is.na(dupRows[1]) != sum(is.na(dupRows[2])))) {
    return(id)
  }
}
...
getIdsDifferingByNA <- function(id_list, df, id_type) {
  ...
  for (id in id_list) {
    dupRows <- getDupRows(id, df, id_type)
    idsDifferentByNA <- append(idsDifferentByNA, identifyRowsDifferingbyNA(id, dupRows), length(idsDifferentByNA))
  }
...
ped %>%
  filter(!(horse_ueln %in% uelns_differbyna) & horse_ueln %in% dup_uelns) %>%
  arrange(horse_ueln)

ped %>%
  filter(!(horse_id %in% ids_differbyna) & horse_id %in% dup_ids) %>%
  arrange(horse_id)

removeRowsWithLessData <- function(id_list, df, id_type) {
  li_toremove <- c()
  for (id in id_list) {
  ...
  return(df[-li_toremove,])
}

ped <- removeRowsWithLessData(ids_differbyna, ped, "horse_id")
...
ped %>%
  filter(horse_id %in% dup_ids) %>%
  arrange(horse_id)
# why have some duplicates that differ only by NA been missed? FIX THIS SO ONLY DAMUELN IS NONZERO
ped %>%
  filter(horse_ueln %in% dup_uelns) %>%
  arrange(horse_ueln)
...
The current dataset will be set aside, so that informative missingness can be assessed at a later point. Ultimately, I will not use dams in the final analysis - I will restrict myself to sires. This is because records for dams are error-prone: mislabelled sexes and multiple dam records for single individuals are frequent. 
...
pass4 <- compareAllDupRows(dup_ids, ped, "horse_id") + compareAllDupRows(dup_uelns, ped, "horse_ueln")
pass4

ped %>%
  filter((horse_id %in% dup_ids) | horse_ueln %in% dup_uelns) 
...
This is the second inconsistency I have observed in this dam data, and thus I have chosen not to take it forward to future analyses. I also remove the now unnecessary `dob`, `sire_ueln` and `sire_id` columns.
...
There are 85 sires listed in the `sire` column that do not yet have their own record. Here I give these individuals their own rows, with everything other than ID set to NA.
